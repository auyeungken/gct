...
var publicAccount =  config.testAccount1; 
var psTargetAccount = config.testAccount2; 
var nonPsAccount = config.testAccount3;
...
contract('GCToken Test', function(accounts) {
    it("icoEnd must be setted",async function(){
        await gct.icoEndTime({from:publicAccount}).then(function (r){
    ...
    */
    ...
    it("privateInvestLockToken must bet setted",async function(){
        await gct.privateInvestLockToken({from:publicAccount}).then(function (r){
    ...
    it("privateSaleTransferRelease must be setted",async function(){
        await gct.privateSaleReleaseTime({from:publicAccount}).then(function (r){
    ...
    it("Cap amount must be equal",async function(){
        await gct.cap({from:publicAccount}).then(function (r){
            assert.equal(r.toNumber(), config.cap,"cap amount not equal");
        }); 
    });

    it("Has already been initialize",async function(){
        await gct.hasInit({from:publicAccount}).then(function (r){
            assert.isTrue(r);
        }); 
    });
    ...
    it("Should not able to set IcoEndTime accept by contract",async function(){            
        var icoEndTime = Math.floor(Date.now()/1000) + (config.icoEndinMinute * 60);
        await gct.setIcoEndTime(icoEndTime, {from: config.ownerAccount}).then(function (r){
    ...
    */
    ...
    it("Should able to do transfer for non PS account",async function(){
        ...
        time = Math.floor(Date.now()/1000);// + (config.icoEndinMinute * 60);
        ...
        await gct.transfer(psTargetAccount, getNonPSBal, {from: nonPsAccount}).then(function (r){
            assert.isTrue(true);
        }).catch(function(e){
